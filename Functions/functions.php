 <?php
    ...
    function getFarmerUsername()
    {
        if (isset($_SESSION['phonenumber'])) {
            ...
            // Query to fetch information from farmerregistration table
            $query_farmer = "SELECT farmer_name, gender 
                             FROM farmerregistration 
                             WHERE farmer_phone = $phonenumber
                             LIMIT 1"; // Assuming one farmer per phone number
    
            $result_farmer = mysqli_query($con, $query_farmer);
    
            if ($result_farmer && mysqli_num_rows($result_farmer) > 0) {
                $row_farmer = mysqli_fetch_array($result_farmer);
                $farmer_name = $row_farmer['farmer_name'];
                $farmer_name = "Hello! " . $farmer_name;
    
                // Display the username
                echo "<label style='color:white; padding-top:7px;'>$farmer_name</label>";
    
                // Call the function to display the profile image
                displayProfileImage($row_farmer['gender']);
            } else {
                // Handle error or display a default message
                echo "User not found or no information available.";
            }
        } else {
    ...
    function displayProfileImage($gender)
    {
        global $con;
    
        // Query to fetch profile image based on gender from the profile table
        $query_profile = "SELECT p.image_data 
                          FROM profile p
                          JOIN farmerregistration fr ON p.name = fr.gender
                          WHERE fr.gender = '$gender'
                          LIMIT 1"; // Assuming one profile image per gender
        ...
        if ($result_profile && mysqli_num_rows($result_profile) > 0) {
            ...
            $profile_image = $row_profile['image_data'];
        } else {
            // Handle error or display a default profile image
            echo "<img src='default_profile_image.png' alt='Default Profile Image' style='width:50px; height:50px; border-radius:50%;'>";
            return; // Return to avoid further processing if default image is used
        }
    
        // Display the profile image
        echo "<img src='data:image/png;base64,".base64_encode($profile_image)."' alt='Profile Image' style='width:50px; height:50px; border-radius:50%;'>";
    }
            ...
            echo "
                ...
                <div class='card pb-1 pl-1 pr-1 pt-0' style='height:542px'>
                    ...
                    <div class='card-body pb-0'>
                        ...
                        <p class='card-text mb-2 font-weight-bold'>PRICE:- $product_price Php/kg</p>
                        <div class='row'>
